...
#  ltnc=unique(ltm[txtype!='mRNA']$txid)
 ...
 #stop("stop here for now")  
  ...
  gg <- ggplot(dtc[order(assembly, coding, -count)], aes(fill=cvar, y=count, x=coding)) +
    ...
    scale_fill_manual(values=colors, labels = c("model RefSeq", "Long (known RefSeq)", "Short (known RefSeq)", "Hybrid (known RefSeq)")) +
    facet_grid(~ assembly) +
...
fig <- ggarrange(g1, g2, labels=c("A", "B"), ncol=1, nrow = 2)+theme(plot.margin = margin(t=0.4, r=1, l=1, unit="cm"))


ggsave(fig, file="fig6_pred_nc.svg", width=8.2, height = 4.2, device="svg")
